...
func TestHandleGetBalanceEvent_NoPermission(t *testing.T) {
	...
	controllersSvc.HandleGetBalanceEvent(ctx, nip47Request, dbRequestEvent.ID, checkPermission, publishResponse)
...
func TestHandleGetBalanceEvent_WithPermission(t *testing.T) {
	...
	assert.Equal(t, int64(21000), publishedResponse.Result.(*controllers.GetBalanceResponse).Balance)
