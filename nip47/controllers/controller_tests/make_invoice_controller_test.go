...
func TestHandleMakeInvoiceEvent_NoPermission(t *testing.T) {
	...
	controllersSvc.HandleMakeInvoiceEvent(ctx, nip47Request, dbRequestEvent.ID, checkPermission, publishResponse)
...
func TestHandleMakeInvoiceEvent_WithPermission(t *testing.T) {
	...
	assert.Equal(t, tests.MockTransaction.Invoice, publishedResponse.Result.(*controllers.MakeInvoiceResponse).Invoice)
}
